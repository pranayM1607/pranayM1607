# Hi, I'm M Pranay 👋

### AI Product Management Aspirant | Generative AI Enthusiast | Data Analytics Explorer

I'm a Computer Science & Business Systems undergraduate passionate about building AI-powered products that solve real-world problems. My interests lie at the intersection of Product Management, Generative AI, Data Analytics, and Machine Learning.

I enjoy transforming ideas into practical applications, analyzing user needs, and leveraging AI to create meaningful user experiences.

---

## 🎯 Current Focus

* AI Product Management
* Generative AI Applications
* Product Strategy & Growth
* Data Analytics & Business Intelligence
* Machine Learning Solutions
* User-Centric Product Development

---

## 🛠️ Technical Skills

### Programming

* Python
* SQL

### Data & Analytics

* Power BI
* Pandas
* NumPy
* Data Visualization
* Exploratory Data Analysis (EDA)

### AI & Machine Learning

* Scikit-Learn
* Random Forest
* Gradient Boosting
* NLP
* Generative AI
* Google Gemini

### Development

* Streamlit
* FastAPI
* Git & GitHub
* VS Code

---

## 🚀 Featured Projects

### 🛡️ Phishing Website Detection Extension

Machine Learning-powered Chrome Extension that detects potentially malicious websites using URL-based feature extraction and Random Forest classification.

**Tech Stack:** Python, Scikit-Learn, Flask, JavaScript, Chrome Extensions

---

### 📚 RAG Research Assistant

Retrieval-Augmented Generation (RAG) system that enables users to upload PDFs and ask natural language questions using Google Gemini, LangChain, and FAISS.

**Tech Stack:** Python, Streamlit, LangChain, FAISS, Google Gemini

---

### 📊 E-Commerce Customer Analytics Platform

Interactive analytics solution for understanding customer behavior, business performance, and sales insights through dashboards and data analysis.

**Tech Stack:** Power BI, Python, Data Analytics

---

### 💬 AI Customer Feedback Analyzer

AI-powered platform that analyzes customer reviews, extracts sentiment, identifies trends, and generates actionable insights.

**Tech Stack:** Python, NLP, Streamlit, Gemini AI

---

### 📝 AI-Powered English Meeting Notes Generator

Generative AI application that converts meeting discussions into structured notes, summaries, and key action items.

**Tech Stack:** Python, Generative AI, NLP

---

## 📈 What I'm Learning

* AI Product Management
* Product Discovery & User Research
* Growth Strategy
* GenAI Product Development
* Data-Driven Decision Making

---

## 🌐 Connect With Me

* GitHub: https://github.com/pranayM1607
* LinkedIn: https://www.linkedin.com/in/pranaym1607/
* Portfolio: Coming Soon

---

## 📌 Career Objective

Seeking opportunities in AI Product Management, Product Strategy, Data Analytics, and Generative AI where I can combine technical knowledge with business thinking to build impactful products.

---

⭐ Building AI-powered products, learning continuously, and exploring the future of intelligent systems.
